...
import strata.stream.core.unbounded.WindowPlan;
...
import strata.stream.pipeline.validation.CompletableBatchValidatorFilter;
import strata.stream.pipeline.validation.CompletableValidatorFilter;
import strata.stream.pipeline.validation.GreaterThanZeroCompletableBatchValidator;
...
public
class CompletableBatchPipelineFactory
    ...
    implements ITestPipelineFactory
{
    ...
    public CompletableBatchPipelineFactory()
    {
                ...
                source
                    ...
                    .map(
                        CompletableBatchValidatorFilter.of(
                            "Long>0",
                            GreaterThanZeroCompletableBatchValidator.of(executor)))
                    .flatMap(stage -> stage.flatMap(list -> list))
                    .map(
                    ...
                    .sinkTo(CompletableContextHistorySink.of(pending)),
                pending);
    }
}
